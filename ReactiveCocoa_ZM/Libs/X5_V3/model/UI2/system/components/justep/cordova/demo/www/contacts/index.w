<?xml version="1.0" encoding="utf-8"?>

<div xmlns="http://www.w3.org/1999/xhtml" component="$UI/system/components/justep/window/window">
  <div class="panel panel-primary cordova-testunit">
    <div class="panel-heading cordova-testunit">
      <div class="panel-title text-center">
        <span style="position: absolute;left: 5px;" data-bind="click:backHome">
          <i xid="i1" class="glyphicon glyphicon-chevron-left"/>
        </span>
        <span>Contacts</span>
      </div>
    </div>
    <div class="panel-content"/>
  </div>  
  <div id="info"> 
    <b>Results:</b>
    <br/>  
    <span id="contacts_results"></span> 
  </div>  
  <h2>Action</h2>  
  <div class="btn btn-default btn-default getContacts">Get phone's contacts</div>  
  <div class="btn btn-default btn-default addContact">Add a new contact 'Dooney Evans'</div>  
</div>
