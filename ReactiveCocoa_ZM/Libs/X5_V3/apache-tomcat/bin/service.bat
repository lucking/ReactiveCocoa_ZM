...
cd ..
set JUSTEP_HOME=%cd%
set JRE_HOME=%JUSTEP_HOME%\java\jre1.7
set JAVA_HOME=
cd %CURRENT_DIR%

rem Set default Service name
set SERVICE_NAME=X5
set DISPLAYNAME=Justep %SERVICE_NAME%
...
:displayUsage
echo.
echo Usage: service.bat install/remove bit(x86/x64) [service_name]
goto end
...
if exist "%CATALINA_HOME%\bin\tomcat6-%BIT%.exe" goto okHome
rem CD to the upper dir
cd ..
set "CATALINA_HOME=%cd%"
:gotHome
if exist "%CATALINA_HOME%\bin\tomcat6-%BIT%.exe" goto okHome
echo The tomcat6-%BIT%.exe was not found...
...
set "EXECUTABLE=%CATALINA_HOME%\bin\tomcat6-%BIT%.exe"
...
"%EXECUTABLE%" //IS//%SERVICE_NAME% ^
    --Description "Justep Application Server (Apache Tomcat 6.0.41) - http://tomcat.apache.org/" ^
